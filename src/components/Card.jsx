...
import { PencilAltIcon, StarIcon, CheckCircleIcon, TrashIcon } from "@heroicons/react/outline";
...
const Card = ({ task, updateTaskStatus, deleteTask, setEditingTask, setIsModalOpen, searchQuery }) => {
  ...
  return (
    <div className={`p-6 rounded-xl border border-[#E0E0E0] flex mb-6 w-full max-w-[390px] bg-[#FFFFFF] shadow-lg hover:shadow-xl transition-shadow duration-200 ${
      ...
      (task.category && task.category.toLowerCase().includes(searchQuery.toLowerCase()))) ? 'ring-2 ring-yellow-400' : ''
    }`}>
      ...
      <div className="flex flex-col flex-grow pr-4 overflow-hidden">
        ...
        <span className="text-xs font-semibold mb-3 self-start flex items-center gap-2 text-[#3E3F5B]">
          {categoryEmoji} {highlightText(categoryName)}
        </span>
